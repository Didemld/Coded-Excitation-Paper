...
p = single(p);
%% Reduce image region
Nx_image= 1;%389;%260;%204;
Ny_image= 82;%200;%315;%630;%366;%240;%238;
...
nx_origin = 180+(Nx-180)/2-Nx_image/2-300;%587;%442;%200;%252;%169;200-
% nx_origin = 180+(Nx-180)/2-Nx_image/2-250;%587;%442;%200;%252;%169;200-
ny_origin =Ny/2-Ny_image/2 ;%137;%139;
% ny_origin =Ny/2-Ny_image/2-50 ;%137;%139;
...
% g_receive = reshape(g_receive,[],size(g_receive,3),1);
...
for i =1:c_num
    ...
    O_c(i) = 1; 
    G_tmp = makeG(g_receive,g_transmit,O_c,p,L,numelements,M);
    G_tmp = permute(G_tmp,[1,3,2]);
    G_tmp = reshape(G_tmp,[],size(G_tmp,3),1);
    G_all{i} = G_tmp;
end
timecase0 =toc(timer0)
...
for i = 1:c_num  
    ...
    for j = i:c_num
        ...
        A = G_all{i}'*G_all{j}; %externel function mtimesx, faster matrix calculation
        A_tr(i,j) = trace(A);
        A_tr(j,i) = A_tr(i,j);       
        A_var{i,j} = A;
        A_var{j,i} = A_var{i,j};
...
timecase1 = toc(timer1)

%% finding the eigenvalues
A_tr = gather(A_tr); %save A_tr to calculate the encoding matrix .