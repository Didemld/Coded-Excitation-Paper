...
addpath('/mnt/Data1/didem/encoding_matrix_code_MacOS/mtimesx')
didem_initialize_k
%% Source = burst
...
code_length = 5;  %% bit length. could be 3, 5, 7 or even more, in the thesis I chose 5.
...
p = single(p);
%% Reduce image region
Nx_image= 120;%389;%260;%204;
Ny_image= 128;%200;%315;%630;%366;%240;%238;



%% (Delay model)
downsample_rate =4; %% further reduce image matrix size
...
for i = 1:c_num
    ...
    for j = i:c_num
        ...
        tic 
        A = mtimesx(G_1,G_2,'SPEEDOMP'); %externel function mtimesx, faster matrix calculation
        toc
        A_tr(i,j) = trace(A);               
    end
