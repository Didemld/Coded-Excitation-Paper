...
% close all
[ind_row2,ind_col2] =find(small_scatter_region(:,:) ==1);
...
for i = 1:9
    load(['I_lesa_1x11', num2str(i), '.mat']);
    I_lesa(:,i) = I_matchedfilter;
end 
...
xlabel('x (mm)')
ylabel('z (mm)')
title('FIM-Opt, below',['SNR = ', num2str(SNR_Fim9)]);
%colorbar
% title('optimized compounding')
axis equal; axis tight;
colorbar
...
I_sig_lesa = I_mid(ind_row,ind_col);

I_noise_lesa=I_mid(ind_noise1,ind_noise2);

SNR_lesa = 10*log10(mean(mean(abs(I_sig_lesa).^2))/mean(mean(abs(I_noise_lesa).^2)))

% Calculate the mean intensity of signal and noise
mu_signal = mean(mean(abs(I_sig_lesa)));    % Mean signal intensity
mu_noise = mean(mean(abs(I_noise_lesa)));  % Mean noise intensity

% Calculate the standard deviation of noise
sigma_noise = std(abs(I_noise_lesa(:)));   % Standard deviation of noise intensities

% Compute CNR
CNR_lesa = (mu_signal - mu_noise) / sigma_noise;
...
xlabel('x (mm)')
ylabel('z (mm)')
title('FIM-Opt,above',['SNR = ', num2str(SNR_lesa)]);
%colorbar
% title('optimized compounding')
axis equal; axis tight;
colorbar
