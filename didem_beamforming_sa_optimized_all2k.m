clc
    ...
    code_length =3;
    load A_tr_192_1x1_code3.mat
    load A_var_192_1x1_code3.mat
    A_real = real(A_tr);
    [V,D] = eig(A_real);  % calculate eigenvectors and eigenvalues of A
    [lambda, idx] = max(diag(D));  % find largest eigenvalue and its index
for tran = 1:5
didem_2k
     load C_192_1x1_code3.mat
    [c_best,C_best,trinv] = C_random_rounding_tr(C,A_var,A_tr,100);
...
%     n1 = wgn(length(tgc),1,-4)/400;
    ...
    filename1 =['I_3EIG' num2str(tran)];
    save([filename1 '.mat'],'I_matchedfilter');
    filename2 =['I_3EIGn' num2str(tran)];
