clc
    ...
    code_length =2;
    load A_tr_256_3x3_code2.mat
    A_real = real(A_tr);
    [V,D] = eig(A_real);  % calculate eigenvectors and eigenvalues of A
    [lmbda, idx] = max(diag(D));  % find largest eigenvalue and its index
for tran = 1:5   
    didem_initialize_k
    
    load C_256_3x3_code2_traceinv2.mat
%     [c_best,C_best] = C_random_rounding(C,A_var,A_tr,1);

    [U,S,V]=svd(C);
    C_rank1 = U(:,tran)*S(tran,tran)*V(:,tran)';
    
    c_best = sqrt(diag(C_rank1)).*sign(C_rank1(:,1));
    ...
    C_m = reshape(c_normalized,numelements,code_length);
    Code = C_m*100;
    %%
...
%     n1 = wgn(length(tgc),1,-4)/400;
    ...
    filename1 =['I_trdd' num2str(tran)];
    save([filename1 '.mat'],'I_matchedfilter');
    filename2 =['I_trddn' num2str(tran)];
