...
%     n1 = wgn(length(tgc),1,-4)/400;
    ...
    filename1 =['I_2gd11' num2str(tran)];
    save([filename1 '.mat'],'I_matchedfilter');
    filename2 =['I_2gdn11' num2str(tran)];
...
Ir=reshape(real(Bmode),Ny_image,Nx_image)';
imagesc(IMAGE_yaxis,IMAGE_xaxis,Ir);
caxis([-rt 0])
colorbar
title('optimized')
xlabel('(mm)')
...
Ir=reshape(real(Bmode_noise),Ny_image,Nx_image)';
imagesc(IMAGE_yaxis,IMAGE_xaxis,Ir);
caxis([-rt 0])
title('SNR = 10dB')
impixelinfo
...
colorbar
subplot(1,3,3)
imagesc(IMAGE_yaxis,IMAGE_xaxis,scatter_region(nx_origin:nx_origin+Nx_image,ny_origin+10:ny_origin+Ny_image-10));
axis equal; axis tight;
colorbar
